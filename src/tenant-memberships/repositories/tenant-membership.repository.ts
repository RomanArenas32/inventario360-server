...
export class TenantMembershipRepository {
  ...
  save(data: { userId: string; tenantId: string; role: TenantRole }): Promise<TenantMembership> {
  ...
  findByUserId(userId: string): Promise<TenantMembership[]> {
    return this.repo.find({ where: { userId, isActive: true }, relations: { tenant: true } });
  }
